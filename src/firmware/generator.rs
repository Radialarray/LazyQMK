...
impl<'a> FirmwareGenerator<'a> {
    ...
    /// Generates config.h merged with keyboard's existing config.
    ///
    /// Reads the keyboard's config.h and appends our definitions.
    fn generate_merged_config_h(&self) -> Result<String> {
        let mut content = String::new();

        // Try to read keyboard's existing config.h
        if let Some(qmk_path) = &self.config.paths.qmk_firmware {
            let keyboard_config = qmk_path
                .join("keyboards")
                .join(&self.config.build.keyboard)
                .join("config.h");

            if keyboard_config.exists() {
                // Read and include keyboard config comments
                content.push_str("// Original keyboard config.h included above\n");
                content.push_str("// Additional keymap-specific configuration below:\n\n");
            }
        }
        ...
        // Define RGB_MATRIX_LED_COUNT if keyboard has RGB
        if self.geometry.has_rgb_matrix() {
            let led_count = self.mapping.key_count();
            content.push_str("// RGB Matrix LED count\n");
            content.push_str(&format!("#define RGB_MATRIX_LED_COUNT {}\n", led_count));
        }
