...
impl<'a> FirmwareGenerator<'a> {
    ...
    pub fn generate(&self) -> Result<(String, String)> {
        // Generate config.h (if needed for RGB matrix)
        if self.geometry.has_rgb_matrix() {
            let config_h = self.generate_config_h()?;
            self.write_config_h(&config_h)?;
        }
    ...
    /// Generates config.h with RGB matrix LED count.
    ///
    /// Creates a config.h file to define RGB_MATRIX_LED_COUNT.
    fn generate_config_h(&self) -> Result<String> {
        let mut code = String::new();

        // File header
        code.push_str("// Generated by keyboard_tui\n");
        code.push_str(&format!("// Layout: {}\n", self.layout.metadata.name));
        code.push_str("// This file supplements the keyboard's config.h\n");
        code.push_str("\n");
        ...
        let led_count = self.mapping.key_count();
        code.push_str(&format!("// RGB Matrix LED count\n"));
        code.push_str(&format!("#define RGB_MATRIX_LED_COUNT {}\n", led_count));

        Ok(code)
    }

    /// Writes config.h to the output directory.
    fn write_config_h(&self, content: &str) -> Result<String> {
        let output_dir = self.get_keymap_directory()?;
        let config_path = output_dir.join("config.h");

        // Atomic write using temp file
        let temp_path = output_dir.join("config.h.tmp");
        fs::write(&temp_path, content)
            .with_context(|| format!("Failed to write {}", temp_path.display()))?;
