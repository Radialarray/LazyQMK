...
impl<'a> FirmwareGenerator<'a> {
    ...
    /// Keys are ordered by LED index as required by QMK.
     fn generate_keymap_c(&self) -> Result<String> {
        ...
        code.push_str(&format!(
            "// Keyboard: {}\n",
            self.layout.metadata.keyboard.as_deref().unwrap_or("unknown")
        ));
        code.push_str(&format!(
            "// Layout Variant: {}\n",
            self.layout.metadata.layout_variant.as_deref().unwrap_or("unknown")
        ));
        code.push('\n');
        ...
        for (layer_idx, layer) in self.layout.layers.iter().enumerate() {
            ...
            code.push_str(&format!(
                "[{}] = {}(",
                layer_idx,
                self.layout.metadata.layout_variant.as_deref().unwrap_or("LAYOUT")
            ));
    ...
    fn get_keymap_directory(&self) -> Result<std::path::PathBuf> {
        ...
        let keyboard = self.layout.metadata.keyboard.as_deref()
            .context("Keyboard not set in layout metadata")?;
        let keymap = self.layout.metadata.keymap_name.as_deref()
    ...
    fn generate_merged_config_h(&self) -> Result<String> {
        ...
        if brightness_255 < 255 {
            ...
            content.push_str(&format!("#define RGB_MATRIX_MAXIMUM_BRIGHTNESS {}\n", brightness_255));
        }
