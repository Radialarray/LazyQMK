...
//!
//! Sub-modules:
//! - `bootloader_combo` — regression tests for the bootloader-combo
//!   decoupling (LazyQMK-epl0.5).
...
pub(super) fn create_test_setup() -> (
    Layout,
    KeyboardGeometry,
    VisualLayoutMapping,
    Config,
...
#[test]
fn test_rules_mk_placeholder_when_no_features() {
    // Regression test for LazyQMK-msc8: a stale rules.mk from a previous
    // combo-enabled deployment can leak COMBO_ENABLE = yes into a freshly
    // generated keymap that no longer needs it. To keep the deploy step
    // idempotent, rules.mk is now ALWAYS emitted — with a placeholder body
    // when no features need enabling — so the deploy overwrites any stale
    ...
    assert!(
        !rules_mk.is_empty(),
        "rules.mk must always be emitted (placeholder when empty) to avoid stale \
         flags from a previous deploy leaking into the new build; got: {rules_mk}"
    );
    assert!(
    ...
    assert!(
        rules_mk.contains("# No QMK features require explicit flags"),
        "placeholder body should explain that no features are enabled, got: {rules_mk}"
    );
    assert!(
...
#[test]
fn test_rules_mk_combos_disabled_no_enable_placeholder() {
    let (mut layout, geometry, mapping, config, keycode_db) = create_test_setup();
    ...
    assert!(
        !rules_mk.is_empty(),
        "rules.mk must still be emitted (placeholder) when combos are disabled"
    );
    assert!(
        !rules_mk.contains("COMBO_ENABLE"),
        "rules.mk should not contain COMBO_ENABLE when combos are disabled, got: {rules_mk}"
    );
}

#[test]
fn test_rules_mk_only_placeholder_combos_no_enable_placeholder() {
    let (mut layout, geometry, mapping, config, keycode_db) = create_test_setup();
    ...
    assert!(
        !rules_mk.is_empty(),
        "rules.mk must still be emitted (placeholder) when only placeholder combos exist"
    );
    assert!(
        !rules_mk.contains("COMBO_ENABLE"),
        "rules.mk should not enable COMBO_ENABLE for placeholder-only combos, got: {rules_mk}"
    );
}
