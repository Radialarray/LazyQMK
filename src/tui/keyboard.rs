...
impl KeyboardWidget {
    ...
    pub fn render(f: &mut Frame, area: Rect, state: &AppState) {
        ...
        for (grid_row_idx, row_data) in grid.iter().enumerate() {
            let cells: Vec<Cell> = row_data.iter().enumerate().map(|(grid_col_idx, text)| {
                // Map back to actual position
                let actual_row = grid_row_idx + start_row;
                let actual_col = grid_col_idx + start_col;
                
                let is_selected = 
                    actual_row == state.selected_position.row as usize &&
                    actual_col == state.selected_position.col as usize;
                
                // Find the key at this position to get its color
                let key_color = layer.keys.iter()
                    .find(|k| k.position.row as usize == actual_row && k.position.col as usize == actual_col)
                    .map(|key| {
