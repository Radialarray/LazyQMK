...
impl crate::tui::component::Component for LayoutPicker {
    ...
    fn handle_input(&mut self, key: crossterm::event::KeyEvent) -> Option<Self::Event> {
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                if let Some(layout) = self.state.get_selected() {
                    Some(LayoutPickerEvent::LayoutSelected(layout))
                } else {
                    None
                }
            }
...
impl crate::tui::component::Component for KeyboardPicker {
    ...
    fn handle_input(&mut self, key: crossterm::event::KeyEvent) -> Option<Self::Event> {
        ...
        match key.code {
            ...
            KeyCode::Enter => {
                if let Some(keyboard) = self.state.get_selected() {
                    Some(KeyboardPickerEvent::KeyboardSelected(keyboard))
                } else {
                    None
                }
            }
