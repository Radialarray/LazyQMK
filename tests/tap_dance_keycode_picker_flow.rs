...
use lazyqmk::models::{KeyDefinition, KeyboardGeometry, Layer, Layout, Position, RgbColor, VisualLayoutMapping};
...
fn test_selecting_td_from_picker_opens_form() {
    ...
    assert!(matches!(state.active_component, Some(ActiveComponent::KeycodePicker(_))));
    ...
    assert_eq!(params.len(), 4, "TD() should have 4 parameters (name, single, double, hold)");
    assert_eq!(params[0].param_type, lazyqmk::keycode_db::ParamType::TapDance);
}
...
fn test_td_form_initialization() {
    ...
    assert!(matches!(state.active_component, Some(ActiveComponent::TapDanceForm(_))));
    assert_eq!(state.active_popup, Some(PopupType::TapDanceForm));
    assert!(matches!(state.tap_dance_form_context, Some(lazyqmk::tui::TapDanceFormContext::FromKeycodePicker)));
}
...
fn test_reproduction_td_selection_flow() {
    ...
    println!("  active_component: {:?}", state.active_component.as_ref().map(|c| std::mem::discriminant(c)));
    println!("  active_popup: {:?}", state.active_popup);
    
    assert!(
        matches!(state.active_component, Some(ActiveComponent::KeycodePicker(_))),
    ...
    let params = state.keycode_db.get_params("TD()");
    assert!(params.is_some(), "TD() should be recognized as parameterized");
    ...
    println!("  active_component: {:?}", state.active_component.as_ref().map(|c| std::mem::discriminant(c)));
    println!("  active_popup: {:?}", state.active_popup);
    println!("  tap_dance_form_context: {:?}", state.tap_dance_form_context);
    println!("  pending_keycode cleared: {}", state.pending_keycode.keycode_template.is_none());
    ...
    assert!(
        matches!(state.tap_dance_form_context, Some(lazyqmk::tui::TapDanceFormContext::FromKeycodePicker)),
